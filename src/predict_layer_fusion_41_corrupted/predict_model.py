...
from src.model_lf_mfcc_4ds.config import XLSR_2B_CONFIGS, FEAT_SEQ_LEN
...
from src.predict_layer_fusion_41_corrupted.csv_dataset import CsvDataset, MyCrop
...
def _predict_model(split: Split, cpus: int, part: int, num_parts: int):
    ...
    model_name = "trained_model_lfds4_wav2vec2-xls-r-2b_10,41"
    ...
    for idx, (corrupted_xlsr_inputs, mos_norm) in enumerate(tqdm(dl)):
        ...
        while j+batch_size <= num_corrupts-skip_last:
            ...
            for _i in range(batch_size):
                if idx < min_N_done[j+_i]:
                    exit_while = True
                    break
            if exit_while:
                break
                
            stacked_inputs = torch.cat(corrupted_xlsr_inputs[j:j+batch_size], dim=0)
            with torch.no_grad():
                output = model(stacked_inputs.to(device), output_hidden_states=True)
            for _i in range(batch_size):
                _xlsr_dev = [my_crop(output.hidden_states[x][_i,:,:]).unsqueeze(0) for x in layers]
                xlsr_devs.append(_xlsr_dev)

            j += batch_size
        while j < num_corrupts:
            if idx < min_N_done[j]:
                xlsr_devs.append(None)
                j += 1
                continue
