...
from src.model_layer_10.config import CONFIGS_PER_XLSR_SIZE, TRAIN_ARGS_PER_XLSR_SIZE
from src.model_layer_10.model_xlsr_layers import Model
from src.train_layer_10.csv_dataset import CsvDataset
...
def _train_model(
    ...
    if force_rezip:
        ...
        extract_features(xlsr_name, layer, Split.TRAIN, ignore_run_once=True)
        extract_features(xlsr_name, layer, Split.VAL, ignore_run_once=True)

    # Trainer parameters.
    layer_name = "_layer_%02i" % layer
    out_name = f"trained_model_{xlsr_name}{layer_name}"
    ...
    model = Model(_configs, layer, _out_csv_path)
...
def train_model(
    ...
    layer_name = "_layer_%02i" % layer
    flag_name = f"trained_model_{xlsr_name}{layer_name}"
