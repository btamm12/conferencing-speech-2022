...
from src.model.config import Config, Transformer, Head
from src.model.config import TRAIN_ARGS_PER_XLSR_SIZE
...
class Model(pl.LightningModule):
    ...
    def __init__(
        ...
        assert xlsr_end_layer > xlsr_start_layer
        self.xlsr_start_layer = xlsr_start_layer
        self.xlsr_end_layer = xlsr_end_layer
        self.num_xlsr_layers = xlsr_end_layer - xlsr_start_layer
        ...
        _models = [
            ...
            for _ in range(self.num_xlsr_layers)
        ]
        _models_str = [
            f"ds{a}_cfg{b}_layer{self.xlsr_start_layer+c}"
            for a in range(self.num_ds)
            for b in range(len(self.configs))
            for c in range(self.num_xlsr_layers)
        ]
        ...
        if self.num_ds == 2:
            for idx in range(self.num_configs * self.num_xlsr_layers):
                ds1_idx = idx + self.num_configs * self.num_xlsr_layers
    ...
    def on_train_epoch_start(self) -> None:
        ...
        # train DL and val DL will each have XLS-R model loaded if caching is False,
        # so this will ensure only one XLS-R model is loaded at a time
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        for idx in range(self.num_models):
            A = self.num_ds
            B = self.num_configs
            C = self.num_xlsr_layers
            ds_idx = (idx % (A*B*C)) // (B*C)
            config_idx = (idx % (B*C)) // C
            xlsr_idx = idx % C // 1
            layer_str = "%02i" % (self.xlsr_start_layer + xlsr_idx)

            _stream = torch.cuda.Stream(device=self.device)
            with torch.cuda.stream(_stream):
                model = self.models[idx]
                out = model.forward(xlsr_states_per_ds[ds_idx][xlsr_idx])
                out = out.squeeze(1)
