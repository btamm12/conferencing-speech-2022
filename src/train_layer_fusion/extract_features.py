...
def extract_features(
    ...
    split_name = str(split).lower().split(".")[1]
    layers_str = ",".join(str(x) for x in layers)
    flag_name = f"extracted_features_{xlsr_name}_layers_{layers_str}_{split_name}"
    ...
    if ignore_run_once:
        ...
        _extract_features(xlsr_name, layers, split)
        return
...
if __name__ == "__main__":
    ...
    for split in [Split.TRAIN, Split.VAL]:
        extract_features(xlsr_name, layers, split)
